    ...
    */
    public static void doComputerTurn (){
        ...
        if (directionY == 0 && directionX < 0) {
            goToLeftOrRight(-1);
            return; //текущий ход был "на 9 часов" относительно предпоследнего, т.е. игрок движется налево
        }
        if (directionY == 0 && directionX > 0) {
            goToLeftOrRight(+1);
            return; //текущий ход на 3, направо
        }
        if (directionY < 0 && directionX == 0) {
            goToUpOrDawn(-1);
            return; //текущий ход на 12, вверх
        }
        if (directionY > 0 && directionX == 0) {
            goToUpOrDawn(+1);
            return; //текущий ход на 6, вниз
        }
        if (directionY < 0 && directionX > 0) {
            goToMinorDiagonal(-1);
            return; //текущий ход на 1.5, вверх и направо
        }
        if (directionY > 0 && directionX < 0) {
            goToMinorDiagonal(+1);
            return; //текущий ход на 7.5, вниз и налево
        }
        if (directionY < 0 && directionX < 0) {
            goToMainDiagonal(-1);
            return; //текущий ход на 10.5, вверх и налево
        }
        if (directionY > 0 && directionX > 0) {
            goToMainDiagonal(+1);
             //текущий ход на 4.5, вниз и направо
